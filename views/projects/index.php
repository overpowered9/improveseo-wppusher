...
<script type="text/javascript">
	...
	function start_build(ids) {
		...
		console.log('Starting build for project:', ids, 'max_iterations:', max_iterations);
		...
		jQuery
			.ajax({
				...
				success: function (data) {
					...
					// If max_iterations is not set or NaN, default to treating as small batch
					if (isNaN(max_iterations) || !max_iterations) {
						max_iterations = 1;
					}
	...
	function start_update(ids) {
		...
		console.log('Starting update for project:', ids, 'max_iterations:', max_iterations);
		...
		jQuery.ajax({
			...
			success: function (data) {
				...
				if (numm_update == data) {
					...
					numm_update = data;
					if (parseInt(data) < max_iterations) {
			...
			error: function(jqXHR, textStatus, errorThrown) {
				...
				jQuery('.show_loading').css("display", "none");
				alert('Failed to update posts. Please check console for errors.');
			}
		});
	}
