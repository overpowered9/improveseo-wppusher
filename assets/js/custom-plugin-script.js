...
jQuery(document).ready(function ($) {
  $("#pop_up_multi_form").submit(function (event) {
    ...
    // Collect keyword-specific image data
    var keywordImagePaths = {};
    jQuery('[id^="keyword_image_path_"]').each(function() {
      var inputId = jQuery(this).attr('id');
      var keyword = jQuery(this).attr('name').replace('keyword_image_paths[', '').replace(']', '');
      var imagePath = jQuery(this).val();
      if (imagePath && imagePath !== '') {
        keywordImagePaths[keyword] = imagePath;
      }
    });
    
    // Add keyword image mappings to form data
    if (Object.keys(keywordImagePaths).length > 0) {
      formData.append('keyword_image_paths', JSON.stringify(keywordImagePaths));
    }
...
// Keyword-specific image upload functionality
function showKeywordSpecificImageUpload() {
  // Hide other upload divs
  jQuery("#multiple_image_div").hide();
  jQuery("#ai-image-generating").hide();
  
  // Show keyword-specific upload div
  jQuery("#keyword_specific_image_div").show();
  
  // Get keywords from the keyword list
  var keywordText = jQuery("#keyword_list").val();
  
  if (!keywordText || keywordText.trim() === '') {
    showImproveSEONotification(
      'warning',
      'No Keywords Found',
      'Please select or add keywords in Step 1 before uploading images.',
      null
    );
    // Reset radio to AI image
    jQuery("#AI_image_multi").prop('checked', true);
    jQuery("#keyword_specific_image_div").hide();
    jQuery("#ai-image-generating").show();
    return;
  }
  
  // Split keywords and filter empty lines
  var lines = keywordText.split('\n');
  var keywords = lines.filter(function(line) {
    return line.trim().length > 0;
  });
  ...
  if (keywords.length === 0) {
    showImproveSEONotification(
      'warning',
      'No Valid Keywords',
      'No valid keywords found. Please add at least one keyword.',
      null
    );
    return;
  }
  
  // Generate upload fields for each keyword
  generateKeywordImageFields(keywords);
}

function generateKeywordImageFields(keywords) {
  var container = jQuery("#keyword_image_list");
  container.empty();
  
  keywords.forEach(function(keyword, index) {
    var keywordTrimmed = keyword.trim();
    var sanitizedKeyword = keywordTrimmed.replace(/[^a-z0-9]/gi, '_').toLowerCase();
    
    var fieldHTML = `
      <div class="keyword-image-item" data-keyword="${keywordTrimmed}" style="background: white; padding: 15px; border-radius: 6px; border: 1px solid #ddd;">
        <div style="display: flex; align-items: center; gap: 15px; flex-wrap: wrap;">
          <div style="flex: 1; min-width: 200px;">
            <strong style="color: #23282d; display: block; margin-bottom: 5px;">Keyword ${index + 1}:</strong>
            <span style="color: #666;">${keywordTrimmed}</span>
          </div>
          <div style="flex: 2; display: flex; align-items: center; gap: 10px;">
            <input type="file" 
                   id="keyword_image_${sanitizedKeyword}" 
                   name="keyword_images[${sanitizedKeyword}]" 
                   accept="image/*" 
                   style="display: none;"
                   onchange="handleKeywordImageUpload(this, '${keywordTrimmed}')">
            <label for="keyword_image_${sanitizedKeyword}" 
                   class="styling_post_page_action_buttons" 
                   style="margin: 0; cursor: pointer; padding: 8px 20px; font-size: 14px;">
              Choose Image
            </label>
            <div id="preview_${sanitizedKeyword}" style="display: none; position: relative;">
              <img src="" alt="Preview" style="max-width: 80px; max-height: 80px; border-radius: 4px; border: 2px solid #ddd;">
              <button type="button" 
                      onclick="removeKeywordImage('${sanitizedKeyword}')" 
            ...
            <input type="hidden" 
                   id="keyword_image_path_${sanitizedKeyword}" 
                   name="keyword_image_paths[${keywordTrimmed}]" 
                   value="">
          </div>
        </div>
      </div>
...
function handleKeywordImageUpload(input, keyword) {
  var sanitizedKeyword = keyword.replace(/[^a-z0-9]/gi, '_').toLowerCase();
  
  if (input.files && input.files[0]) {
    var file = input.files[0];
    
    // Show loading
    showImproveSEOLoading('Uploading Image', 'Please wait while we upload your image...');
    ...
    jQuery.ajax({
      ...
      processData: false,
      success: function(response) {
        hideImproveSEOLoading();
        
        if (response.success) {
          // Show preview
          var previewDiv = jQuery("#preview_" + sanitizedKeyword);
          previewDiv.find('img').attr('src', response.data.url);
          previewDiv.show();
          
          // Store the uploaded image URL
          jQuery("#keyword_image_path_" + sanitizedKeyword).val(response.data.url);
          
          showImproveSEONotification(
            'success',
            'Image Uploaded',
            'Image for "' + keyword + '" uploaded successfully.',
            null
          );
        } else {
          showImproveSEONotification(
...
function removeKeywordImage(sanitizedKeyword) {
  jQuery("#preview_" + sanitizedKeyword).hide();
  jQuery("#keyword_image_path_" + sanitizedKeyword).val('');
  jQuery("#keyword_image_" + sanitizedKeyword).val('');
}
