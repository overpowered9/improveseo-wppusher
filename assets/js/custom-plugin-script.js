...
function generateBulkKeywordImageSelection(keywords) {
    ...
    if (keywords.length === 0) {
        container.html('<p>No keywords found. Please go back and enter keywords.</p>');
        return;
    }
    ...
    keywords.forEach((keyword, index) => {
        ...
        const html = `
            <div class="keyword-image-item" data-keyword-index="${index}" style="border: 1px solid #ddd; padding: 20px; margin-bottom: 20px; border-radius: 5px; background: #f9f9f9;">
                <h4 style="margin-bottom: 15px; color: #333;">Keyword: <strong>${escapeHtml(keywordTrimmed)}</strong></h4>
                <div class="image-method-selection" style="margin-bottom: 15px;">
                    <label style="display: block; margin-bottom: 10px;">
                        <input type="radio" name="image_method_${index}" value="AI_image_one" 
                               id="ai_method_${index}" checked style="margin-right: 8px;">
                        <span style="font-weight: 500;">Generate Image Using AI</span>
                    </label>
                    
                    <label style="display: block;">
                        <input type="radio" name="image_method_${index}" value="Multiple_images" 
                               id="upload_method_${index}" style="margin-right: 8px;">
                        <span style="font-weight: 500;">Upload Image</span>
                    </label>
                </div>
                
                <div class="upload-area-${index}" style="display:none; margin-top:15px; padding: 15px; background: #fff; border: 2px dashed #ccc; border-radius: 5px;">
                    <input type="file" class="keyword-file-input" 
                           data-index="${index}" accept="image/*" style="margin-bottom: 10px;">
                    <div class="image-preview-${index}" style="margin-top: 10px;"></div>
                    <button type="button" class="btn-delete-image" data-index="${index}" 
                            style="display:none; margin-top: 10px; padding: 8px 15px; background: #dc3545; color: white; border: none; border-radius: 3px; cursor: pointer;">Delete Image</button>
...
// Handle radio button toggle for image method selection
jQuery(document).on('change', 'input[name^="image_method_"]', function() {
    const index = jQuery(this).attr('name').match(/\d+/)[0];
    const uploadArea = jQuery('.upload-area-' + index);
    
    if (jQuery(this).val() === 'Multiple_images') {
        uploadArea.slideDown(300);
    } else {
        uploadArea.slideUp(300);
        // Clear uploaded image if switching back to AI
        jQuery('input[name="keyword_image_url_' + index + '"]').val('');
        jQuery('.image-preview-' + index).empty();
        jQuery('.btn-delete-image[data-index="' + index + '"]').hide();
        jQuery('.keyword-file-input[data-index="' + index + '"]').val('');
    }
});

// Handle file upload per keyword
jQuery(document).on('change', '.keyword-file-input', function() {
    ...
    jQuery('.image-preview-' + index).html('<p>Uploading image...</p>');
    ...
    jQuery.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                // Show preview
                jQuery('.image-preview-' + index).html(
                    '<img src="' + response.data.image_url + '" style="max-width: 200px; max-height: 200px; border: 1px solid #ddd; border-radius: 3px;">'
                );
                jQuery('.btn-delete-image[data-index="' + index + '"]').show();
            } else {
...
// Handle delete image
jQuery(document).on('click', '.btn-delete-image', function() {
    const index = jQuery(this).data('index');
