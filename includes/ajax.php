...
add_action('wp_ajax_deduct_keyword_credits', 'deduct_keyword_credits');

function deduct_keyword_credits() {
    // Get API credentials
    ...
    if (empty($api_key) || empty($site_code)) {
        wp_send_json_error(array(
            'message' => 'Missing API credentials. Please configure your API Key and Site Code in ImproveSEO settings.'
        ));
        return;
    }
    
    // Get keyword count
    $keyword_count = isset($_POST['keyword_count']) ? intval($_POST['keyword_count']) : 0;
    
    if ($keyword_count <= 0) {
        wp_send_json_error(array(
            'message' => 'Invalid keyword count'
        ));
        return;
    }
    ...
    $api_endpoint = $admin_server_url . '/api/v1/generate/generatekeywordded';
    ...
    if (is_wp_error($response)) {
        wp_send_json_error(array(
            'message' => 'Failed to connect to server: ' . $response->get_error_message()
        ));
        return;
    }
    ...
    if ($status_code === 200) {
        ...
        $error_message = isset($result['error']) ? $result['error'] : 'Failed to deduct credits';
        wp_send_json_error(array(
