...
/**
 * Inject FAQPage JSON-LD into the head of singular posts generated by ImproveSEO.
 *
 * Parses the FAQ pairs straight from the post content (no dependency on the save flow), so it works for
 * both single and bulk v2 posts. Only runs on our generated content (identified by the wrapper class).
 */
function improveseo_inject_faq_schema() {
    ...
    // Only our generated content (v2 wrapper, or the legacy section wrapper).
    if ( strpos( $post->post_content, 'improveseo-v2-content' ) === false
        && strpos( $post->post_content, 'main-content-section-improveseo' ) === false ) {
        return;
    }
    $faqs = improveseo_extract_faq_pairs( $post->post_content );
    if ( empty( $faqs ) ) {
        return;
    }
    $entities = array();
