...
function improveseo_handle_shortcode($attributes, $content = null, $called = null)
{
    ...
    if (isJSON($shortcode->content)) {
        ...
        // Images
        if (isset($media->id)) {
            return '<img src="' . esc_url($media->url) . '" alt="' . esc_attr($media->tags) . '">';
        }
        // Videos
        if (isset($media->videoId)) {
            return '<iframe type="text/html" width="640" height="390" src="http://www.youtube.com/embed/' . esc_attr($media->videoId) . '" frameborder="0"></iframe>';
        }
    } else return esc_html($shortcode->content);
}
