...
function improve_seo_enqueue_scripts($hook)
{
	...
	$improve_seo_inline_script_3 = "
		(function($) {
			...
			$('.build-project').on('click', function(event) {
			...
			window.build_project = function(id) {
				...
				var loadingBox = $('.show_loading');
				loadingBox.css('display', 'block');
				loadingBox.find('h1').html('Building project.... in progress');
				start_build(id);
			};

			function start_build(ids) {
				console.log('⚙️ start_build() called for ID:', ids);
				var max_iterations = parseInt($('#max-iterations').val()) || 0;
				var export_url = '" . esc_url(admin_url("admin.php?page=improveseo_projects&action=export_preview_url&id=")) . "';
				var loadingBox = $('.show_loading');
				var is_preview_available = $('#is_preview_available').val();
				var numm = 0; // ✅ Fix: Initialize variable
				...
				$.ajax({
					...
					success: function(data) {
						...
						loadingBox.find('h1').html('Building project.... in progress');
						loadingBox.find('h2').html('Posts generated by now ' + data);
						...
						if (max_iterations > 100) {
							...
							if (is_preview_available === 'yes') {
								loadingBox.find('h1').html('Exporting posts. Please wait');
								loadingBox.find('h2').html('');
								window.location.href = export_url + ids + '&noheader=true';
							} else {
								setTimeout(() => {
									loadingBox.hide();
