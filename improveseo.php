...
function workdex_init()
{
	...
	if ($time < (time() - 3600 * 12)) {
		...
		$wpdb->query("UPDATE " . $wpdb->posts . " SET post_status='publish' WHERE post_date<=now() and post_date_gmt<=now()"); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,PluginCheck.Security.DirectDB.UnescapedDBParameter -- fixed query text; the only interpolation is the table name, which cannot be bound
